...
class Transformer:
    ...
    def compile(self, optimizer='adam', active_layers=999):
        ...
        print(tgt_seq.shape, tgt_true.shape)
...
class QANet_Block:
    def __init__(self, dim, n_head, n_conv, kernel_size, dropout=0.1, add_pos=True):
        ...
        self.self_att = MultiHeadAttention(n_head=n_head, d_model=dim, d_k=dim//n_head, d_v=dim//n_head, dropout=dropout, use_norm=False)
...
if __name__ == '__main__':
    ...
    s2s.model.fit([X, Y], None, batch_size=256, epochs=40, validation_split=0.05, callbacks=[TestCallback(), lr_scheduler])
